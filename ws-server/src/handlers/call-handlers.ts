...
export function callHandlers(io: Server, socket: Socket): void {
  // 통화 시작
  ...
  // 통화 수락
  socket.on('call:accept', (data: { sessionId: string; receiverId: string }) => {
    try {
  ...
  // 통화 거절
  socket.on('call:decline', (data: { sessionId: string; receiverId: string; reason?: string }) => {
    try {
  ...
  // 통화 종료
  socket.on('call:end', (data: { sessionId: string; userId: string; reason?: string }) => {
    try {
  ...
  // ICE Candidate
  socket.on('call:ice_candidate', (data: any) => {
    try {
  ...
  // 음성 채널 참가
  socket.on('call:join_channel', async (data: { channelId: string; userId: string }) => {
    try {
  ...
  // 음성 채널 나가기
  socket.on('call:leave_channel', (data: { channelId: string; userId: string }) => {
    try {
