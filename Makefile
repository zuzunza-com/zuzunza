...
build-all: build build-studio build-server
...
app-start: ensure-pm2-user
	...
	su -l $(PM2_USER) -c "DITTO_REPO_ROOT=$(ROOT_DIR) ACTIVE_BUILD=$$ACTIVE_BUILD PM2_HOME=$(PM2_HOME) pm2 restart dyllis-ditto --update-env --max-memory-restart $(PM2_MAX_MEM)" 2>/dev/null || \
